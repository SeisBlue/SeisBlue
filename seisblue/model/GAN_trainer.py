...
from seisblue.model.attention import transformer
from seisblue.model.GAN_model import build_discriminator, build_cgan, build_patch_discriminator
...
class GeneratorTrainer(BaseTrainer):
    ...
    def train_loop(self,
        ...
        if ckpt_manager.latest_checkpoint:
            ...
            print(f'Latest checkpoint epoch {ckpt_manager.latest_checkpoint} restored!!')
        ...
        for epoch in range(epochs):
            ...
            for train in dataset.prefetch(100).batch(batch_size):
                ...
                if n % log_step == 0:
                    ...
                    example = next(seisblue.example_proto.batch_iterator(val))
...
if __name__ == "__main__":
    ...
    tfr_list = seisblue.utils.flatten_list(tfr_list)
    # noise = seisblue.utils.get_dir_list('/home/andy/TFRecord/noise',
    #                                     suffix='.tfrecord')
    # for item in noise:
    #     tfr_list.append(item)

    model_instance = 'demo_gan'
    trainer = GeneratorTrainer(database)
    trainer.train_loop(tfr_list, model_instance,
                       batch_size=256, epochs=15,log_step=5,
                       plot=True)
